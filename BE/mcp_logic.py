...
@mcp.tool(
    name="get_dbt_diaries_by_date",
    description="특정 날짜의 DBT 감정일기 목록을 조회합니다."
)
async def get_dbt_diaries_by_date(
    date: str = Field(..., description="조회할 날짜(YYYY-MM-DD)")
) -> list:
    """
    주어진 날짜(date)에 해당하는 DBT 감정일기(DbtDiary) 목록을 조회합니다.
    """
    response = supabase.table("DbtDiary").select("*").eq("created_at", date).execute()
    return response.data

@mcp.tool(
    name="get_dbt_diaries_by_id",
    description="특정 id의 DBT 감정일기 항목을 조회합니다."
)
async def get_dbt_diaries_by_id(
    id: str = Field(..., description="DBT 감정일기 id")
) -> list:
    """
    주어진 id에 해당하는 DBT 감정일기(DbtDiary) 항목을 조회합니다.
    """
...
async def update_dbt_diary(
    id: int = Field(..., description="수정할 DBT 감정일기 id"),
    date: Optional[str] = Field(None, description="일기 날짜(YYYY-MM-DD)"),
    ...
    """
    id로 지정한 DBT 감정일기 항목의 값을 입력받은 필드만 수정합니다.
    """
    update_fields = {}
    if date is not None:
    ...
    if not update_fields:
        return []  # 아무것도 수정하지 않음
    response = supabase.table(table_name).update(update_fields).eq("id", id).execute()
    return response.data
...
async def delete_dbt_diary(
    id: int = Field(..., description="삭제할 DBT 감정일기 id")
) -> list:
    """
    id로 지정한 DBT 감정일기 항목을 실제로 삭제하지 않고 deleted=true로 변경(soft delete)합니다.
    """
